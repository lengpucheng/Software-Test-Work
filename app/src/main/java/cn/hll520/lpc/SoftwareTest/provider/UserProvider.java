...
public class UserProvider extends ContentProvider {
    public static final String AUTHORITIES= UserProvider.class.getCanonicalName();
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] colmus, @Nullable String selection, @Nullable String[] selectionArgs, @Nullable String sortOrder) {
        ...
        switch (match) {
            case CONTACT:
                ...
                cursor = db.query(WtuOpenHelper.T_USER, colmus, selection, selectionArgs, null, null, sortOrder);
                Log.i("msc", "查询成功");
                break;
            default:
                break;
    ...
    public Uri insert(@NonNull Uri uri, @Nullable ContentValues cv) {
        ...
        switch (match) {
            case CONTACT:
                ...
                long id = db.insert(WtuOpenHelper.T_USER, "", cv);
                if (id != -1) {
                    Log.i("msc", "插入成功！");
                    //拼接生成新的URI
                    uri = ContentUris.withAppendedId(uri, id);
                    //
                    getContext().getContentResolver().notifyChange(UserProvider.URI_CCONTACT,null);
                }
                break;
            default:
                break;
    ...
    public int delete(@NonNull Uri uri, @Nullable String whereClause, @Nullable String[] whereArgs) {
        ...
        switch (match) {
            case CONTACT:
                //获取数据库对象，可写
                SQLiteDatabase db = openHelper.getWritableDatabase();
                deletecount = db.delete(WtuOpenHelper.T_USER, whereClause, whereArgs);
                if (deletecount > 0){
                    Log.i("msc", "删除成功!");
                getContext().getContentResolver().notifyChange(UserProvider.URI_CCONTACT,null);
                }
    ...
    public int update(@NonNull Uri uri, @Nullable ContentValues cv, @Nullable String whereClause, @Nullable String[] whereArgs) {
        ...
        switch (match) {
            case CONTACT:
                //获取数据库对象，可写
                SQLiteDatabase db = openHelper.getWritableDatabase();
                updatecount = db.update(WtuOpenHelper.T_USER, cv, whereClause, whereArgs);
                if (updatecount > 0){
                    Log.i("msc", "更新成功");
                getContext().getContentResolver().notifyChange(UserProvider.URI_CCONTACT,null);}
                break;
